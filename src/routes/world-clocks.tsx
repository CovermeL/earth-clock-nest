...
export const Route = createFileRoute("/world-clocks")({
  head: () => {
    const seo = subPageSeo("/world-clocks", "World Clocks", DESCRIPTION, "WebPage");
    return {
      meta: [
        ...seo.meta,
        { title: TITLE },
...
function WorldClocksPage() {
  ...
  const cities = selected ? [selected, ...CITIES.filter((c) => c.tz !== selected.tz)] : CITIES;

  return (
    <SiteLayout>
      <PageHeader
        eyebrow="World clocks"
        title="Live times around the world"
      ...
      <div className="mt-14 space-y-16">
        <PageSection
          id="search"
          title="Find a city"
          description="Search by city or country to bring it to the top of the grid."
        >
        ...
        <PageSection
          id="grid"
          title="Major cities"
          description="Times update every second and follow each region's daylight saving rules."
        >
        ...
        <PageSection
          id="regions"
          title="Times by region"
          description="A quick look at the busiest business hubs on each continent."
        >
